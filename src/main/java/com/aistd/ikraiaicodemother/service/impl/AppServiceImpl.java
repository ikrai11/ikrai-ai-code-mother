...
import com.aistd.ikraiaicodemother.ai.AiCodeGenTypeRoutingServiceFactory;
...
import com.aistd.ikraiaicodemother.service.ScreenshotService;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App>  implements AppService{
    ...
    public Flux<String> chatToGenCode(Long appId, String message, User loginUser) {
        ...
        if (!app.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "您没有权限访问此应用");
        }
        //4.获取应用的代码生成的类型
        String codeGenTypeStr = app.getCodeGenType();
        CodeGenTypeEnum codeGenTypeEnum = CodeGenTypeEnum.getEnumByValue(codeGenTypeStr);
        ThrowUtils.throwIf(codeGenTypeEnum == null, ErrorCode.PARAMS_ERROR, "应用代码生成类型不存在");
        //5.通过校验后，添加用户消息到对话历史
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        App app = this.getById(appId);
        ThrowUtils.throwIf(app == null, ErrorCode.NOT_FOUND_ERROR, "应用不存在");
        //3.验证用户是否有权限部署应用，仅本人可以部署
        if (!app.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "您没有权限部署此应用");
        }
        //4.检查是否已有deployKey，若有则直接返回
        ...
        if (codeGenTypeEnum == CodeGenTypeEnum.VUE_PROJECT) {
            ...
            ThrowUtils.throwIf(!buildSuccess, ErrorCode.SYSTEM_ERROR, "Vue项目构建失败：请检查项目依赖和代码");
            //检查dist目录是否存在
            File distDir = new File(sourceDirPath,"dist");
            ThrowUtils.throwIf(!distDir.exists()||!distDir.isDirectory(), ErrorCode.SYSTEM_ERROR, "Vue项目构建完成，但dist目录不存在");
            //将dist目录作为部署目录
            sourceDir = distDir;
            log.info("Vue项目构建完成，部署目录为：{}",distDir.getAbsolutePath());
        }
        //8.复制文件到部署目录
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "部署应用失败,请联系管理员"+e.getMessage());
        }
        ...
        boolean upadateResult = this.updateById(updateApp);
        ThrowUtils.throwIf(!upadateResult, ErrorCode.SYSTEM_ERROR, "更新应用部署失败");
        //10.返回可访问的URL
